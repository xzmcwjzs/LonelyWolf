...
namespace MalignantTumorSystem.BLL
{
    public partial class Chronic_disease_Comm_Testing_Urine_AddService : BaseService<Chronic_disease_Comm_Testing_Urine_Add>,IChronic_disease_Comm_Testing_Urine_AddService
    {
        ...
        public bool UpdateSubjective(List<Chronic_disease_Comm_Testing_Urine_Add> subjectiveList, string id)
        {
            int count = CurrentDal.LoadEntities(t => t.urine_id == id).Count();
            if (count > 0)
            {
                CurrentDal.DeleteByLambda(t => t.urine_id == id);
                if (!(Db.SaveChanges() > 0))
                {
                    return false;
                }
