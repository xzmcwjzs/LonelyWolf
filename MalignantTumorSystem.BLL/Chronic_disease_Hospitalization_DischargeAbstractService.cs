...
namespace MalignantTumorSystem.BLL
{
    public partial class Chronic_disease_Hospitalization_DischargeAbstractService : BaseService<Chronic_disease_Hospitalization_DischargeAbstract>, IChronic_disease_Hospitalization_DischargeAbstractService
    {
        ...
        public IQueryable<Chronic_disease_Hospitalization_DischargeAbstract> LoadSearchEntities(Model.SearchParam.MedicalHistory_HospitalizationParam pam)
        {
            int start = (pam.PageIndex - 1) * pam.PageSize + 1;
            int end = pam.PageIndex * pam.PageSize;
            string sql = "select * from (select *,ROW_NUMBER() over (order by create_time) as num from Chronic_disease_Hospitalization_DischargeAbstract where time in (select  MAX(time) from Chronic_disease_Hospitalization_DischargeAbstract group by resident_id) and type like 'Therioma'";
            ...
            string sql2 = "select count(*) from  Chronic_disease_Hospitalization_DischargeAbstract where hospitalization_date in (select  MAX(hospitalization_date) from Chronic_disease_Hospitalization_DischargeAbstract group by resident_id)";
