...
namespace MalignantTumorSystem.BLL
{
    public partial class Chronic_disease_Hospitalization_ExpensesService : BaseService<Chronic_disease_Hospitalization_Expenses>, IChronic_disease_Hospitalization_ExpensesService
    {
        ...
        public IQueryable<Chronic_disease_Hospitalization_Expenses> LoadSearchEntities(Model.SearchParam.MedicalHistory_HospitalizationParam pam)
        {
            int start = (pam.PageIndex - 1) * pam.PageSize + 1;
            int end = pam.PageIndex * pam.PageSize;
            string sql = "select * from (select *,ROW_NUMBER() over (order by create_time) as num from Chronic_disease_Hospitalization_Expenses where hospitalization_date in (select  MAX(hospitalization_date) from Chronic_disease_Hospitalization_Expenses group by resident_id) and type like 'Therioma'";
            ...
            string sql2 = "select count(*) from  Chronic_disease_Hospitalization_Expenses where hospitalization_date in (select  MAX(hospitalization_date) from Chronic_disease_Hospitalization_Expenses group by resident_id)";
